/**
 * @file highscore.c
 * @brief High-score management: load, save, display and input of player names.
 */
...
static hs_entry_t list[MAX_HS_ENTRIES];  /**< Array of high-score entries. */
static int        count = 0;              /**< Number of entries currently stored. */
extern uint8_t    scancode;               /**< Last PS/2 scancode read by ISR. */

/**
 * @brief Convert a PS/2 make-code to its ASCII character.
 * 
 * Ignores break codes and returns '\0' if unmapped.
 * 
 * @param scancode The raw PS/2 make code.
 * @return The corresponding uppercase character, digit or symbol; 0 otherwise.
 */
static char scancode_to_char(uint8_t sc) {
    switch (sc) {
        ...
        case 0x0B: return '0';  case 0x39: return ' ';
        case 0x0C: return '-';  case 0x0D: return '=';  case 0x1A: return '[';
        case 0x1B: return ']';  case 0x27: return ';';  case 0x28: return '\'';
        case 0x29: return '`';  case 0x2B: return '\\'; case 0x33: return ',';
...
/**
 * @brief Load high-scores from disk into memory.
 * 
 * Opens "highscores.dat", reads entry count and entries.
...
/**
 * @brief Save current high-scores list to disk.
 * 
 * Overwrites "highscores.dat" with the entry count and list.
 */
...
/**
 * @brief Attempt to insert a new score into the high-score table.
 * 
 * If the score qualifies, prompts the player for a name via keyboard input
 * and stores the new entry in sorted order.
 * 
  ...
  if (count == 0 || score > list[count-1].score || count < MAX_HS_ENTRIES) {
    ...
    while (true) {
      vg_draw_rectangle(0, 0,
        ...
        mode_info.YResolution, 0x000000);
...
static bool highscore_active = false;  /**< Whether high-score screen is showing. */

/**
 * @brief Activate the high-score display.
 */
...
/**
 * @brief Query if the high-score display is active.
...
/**
 * @brief Render the high-score screen.
 *
 * Draws all entries with rank, name and score. Instructs user to press ESC.
 */
...
/**
 * @brief Handle key press while high-score screen is active.
 * @param scancode PS/2 scancode of the pressed key.
 */
void highscore_handle_key(uint8_t sc) {
    if (sc == ESC_BREAK_CODE) {
